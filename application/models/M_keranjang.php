...
class M_keranjang extends CI_Model{
	...
	function input_pesan($idpesan,$tgl,$status,$iduser,$kirim,$total_pesan,$jatuh_tempo){
		$query = $this->db->query("INSERT INTO `pesan`(`id_pesan`, `tanggal_pesan`, `jatuh_tempo`, `status`, `id_kostumer_id`, `pengiriman_id_kirim`, `total_pesan`) VALUES ('$idpesan','$tgl','$jatuh_tempo','$status','$iduser','$kirim','$total_pesan')");
	}
	function tampil_pesan(){
		$query = $this->db->query("SELECT * FROM pesan JOIN kostumer ON kostumer.id_kostumer=pesan.id_kostumer_id JOIN pengiriman ON pesan.pengiriman_id_kirim=pengiriman.id_kirim");
		return $query->result();
	}
